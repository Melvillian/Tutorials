...
rule uniqueManager(uint256 fundId1, uint256 fundId2, method f) {
	...
	assert getCurrentManager(fundId1) == 0 => !isActiveManager(getCurrentManager(fundId1)), "manager of fund1 is active";
	assert getCurrentManager(fundId2) == 0 => !isActiveManager(getCurrentManager(fundId2)), "manager of fund2 is active";
}
